...
@Component({
  selector: 'app-doctor-user-item',
  templateUrl: './doctor-user-item.component.html',
  styleUrls: ['./doctor-user-item.component.scss']
})
export class DoctorUserItemComponent {
  @Input() userItem!:IDoctorsInterface;
  @Input() senderUserId!:string | null;
...
constructor(private router:Router, private firestore:FireStoreCollectionsServiceService){
  ...
  chatWithDoctor(doc: IDoctorsInterface) {
    this.firestore.addUserIdToFriendsArray(doc.docId,this.senderUserId).subscribe(x=>{
      
    })
    this.firestore.addCurrentUserIdToFriendsArray(doc.docId,this.senderUserId).subscribe(x=>{
      
    })
  this.chatEmitter.emit(doc);
  console.log(doc.docId,this.senderUserId)
  }
